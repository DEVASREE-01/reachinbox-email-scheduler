...
import { CalendarDays, ChevronLeft, ChevronRight, MailWarning } from 'lucide-react';
...
export const ScheduledEmailsTable: React.FC<ScheduledEmailsTableProps> = ({
    ...
    return (
      <div className="w-full h-64 flex items-center justify-center glass-card rounded-xl">
        <div className="flex flex-col items-center gap-3 text-dark-400">
          ...
          <span className="text-sm font-semibold">Loading scheduled emails...</span>
        </div>
      </div>
    );
  }
  ...
  if (emails.length === 0) {
    return (
      <EmptyState
        ...
        description="Schedule your first email campaign to populate this list."
      />
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <Table headers={['Recipient', 'Subject', 'Scheduled Time', 'Status', 'Attempts']}>
        {emails.map((email) => (
          <tr
            key={email.id}
